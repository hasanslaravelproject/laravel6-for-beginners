    ...
    <body>
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <table class="table">
                                ...
                                <td>
                                    <div class="form-group">
                                        <div class="input-group">
                                            <span class="input-group-addon btn btn-info exc-qty"  data-row_id="{{  $item->rowId }}" data-product_id="{{  $item->id }}"><i class="fa fa-minus"></i></span>
                                            <input type="text" class="form-inline text-center qty current-qty_{{$item->id}} "data-row_id="{{  $item->rowId }}" data-product_id="{{  $item->id }}"
                                            style="width:70px" value="{{ $item->qty   }}">
                        ...
                        <tr class="subtotals">
                            <td >Total:<?php echo Cart::subtotal(); ?> {!! Checkout_btn_helper('checkout') !!}</td><br>
        ...
        <script>
            ...
            <script>
                //$('.exc-qty').on('click',function(){
                    $('body').on('click','.exc-qty',function(){
                        var rowId=$(this).data('row_id');
                        var product_id=$(this).data('product_id');
                        var  current_qty=$('.current-qty_'+ product_id).val();
            ...
            //$('.update-qty').on('click',function(){
                $('body').bind('keyup','.qty',function(){

                    var rowId=$('.qty').data('row_id');
